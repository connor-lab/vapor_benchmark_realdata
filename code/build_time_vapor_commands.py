...
for fn in os.listdir("single_full_length_assembled_contigs/"):
    ...
    if seg == "HA":
        if sample_type == "A":
            ...
            ref = "refs/"+B_segs[seg]
        readsf = "reads/"+fn.split("_%s_" % seg)[0] + "_R1_001.fastq.gz"
        outf = "vapor_time_results/"+sample_name+"_"+seg+".vpout"
        # Finally build command
        print("{ time vapor.py -fa %s -fq %s ; } > %s 2>&1" % (ref, readsf, outf))
